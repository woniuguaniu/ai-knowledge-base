...
| 维度 | 局限 |
|---|---|
| 来源单一 | 全文基于**一篇社区文章**（LINUX DO @flymyd）加整理者补充，未与论文或官方文档逐项对照 |
| 样本狭窄 | Dense/MoE 对照只用了 **Qwen3-32B vs Qwen3-30B-A3B** 一组，结论未必推广到其他家族（如 DeepSeek-V3 的细粒度专家、Mixtral 的 8x7B 结构） |
| 时效性 | 参数经验值、"70B 以上几乎都转 MoE"这类判断绑定 2026 年初的模型格局，会随新模型发布失效 |
| 覆盖边界 | 部署侧（数据精度、量化、显卡选型）**已拆到**[本地部署模型量化选型](../03_应用实践/本地部署模型量化选型.md)，本篇不含 |
| 实战验证 | 整理者**未实际训练或部署过 MoE 模型**，架构理解来自资料研读 |
