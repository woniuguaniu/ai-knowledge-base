...
| Claude Code / 桌面版 | 接近 ~95% 时**自动压缩**（默认值，可用环境变量 `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE` 调早），把早期对话摘要化后替换原文 |
...
- [Skill 工程化设计与失效防御](../06_Agent工程/Skill工程化设计与失效防御.md) —— 上下文压缩的**工程后果**：`/compact` 之后规则从上下文消失，Agent 凭残留摘要干活；那篇讲怎么用结构化冗余对抗这件事
...
| 维度 | 客观情况 |
|---|---|
| 时效性 | 含约 21 处具体版本号/型号表述；含约 15 处价格相关表述；文件最后修改于 2026-06-04——版本与价格类信息变动快，引用前请核对当前情况 |
