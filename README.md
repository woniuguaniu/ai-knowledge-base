...
| **LLM Wiki 知识库架构** ⭐新增 | [RAG/LLM-Wiki知识库架构.md](03_应用实践/RAG/LLM-Wiki知识库架构.md) | **「RAG 已死」是炒作吗?——把朴素 RAG 升级为「知识编译系统」**:三层架构(**Raw→Wiki→Schema**,查询时理解→摄入时编译)+ Wiki 层**七大能力**(**页面化 / 层级目录 / 双向链接 / 实体关系 / 自动摘要 / 自动索引 / 语义导航 Retrieval as Agent Action**)+ 动态更新双轮驱动(外部事件 / 内部逻辑)+ **现实挑战**(构建成本高 / 级联失效 / 不适合超实时数据)+ **三大工程陷阱**(实体命名漂移 / 链接过量 / 摘要级联失效)+ 一个在建 RAG 系统的设计草案对照;与 [Embedding-Reranker](03_应用实践/Embedding-Reranker-向量数据库.md) 互补(那篇讲工具,这篇讲知识怎么组织) |
