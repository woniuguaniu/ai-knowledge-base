...
*最后更新: 2026-05-28(同日新增 2 篇笔记 → ① `05_技术基础/静态站点生成器与Quartz部署实战.md`:把这次部署知识库到 `kingrich.top/knowledge-base/quartz/` 的实战流程沉淀成 SSG 范式入门 + 三种部署路线对比 + 完整 7 步 SOP + 常见报错速查 ② `05_技术基础/CI-CD与GitHub-Actions.md`:"推一次代码自动构建测试部署通知"的机制讲透,含 workflow yaml 详解 + 5 大踩坑 + 8 大 CI 工具横评;共 5 处反向回填 → SSG 篇:`云服务交付模型.md` 加 PaaS vs IaaS 对照 / `程序员黑话速查.md` 加 SSG 术语 / `程序小白概念扫盲手册.md` 加进阶指引;CI/CD 篇:`静态站点生成器与Quartz部署实战.md` 加配套引用 / `程序员黑话速查.md` 加 CI/CD 术语)*
