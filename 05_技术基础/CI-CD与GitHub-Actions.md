...
- 本文在正文/文末已有的出处与时间标记：
  - **创建时间**：2026-05-28(继 SSG 部署实战笔记后,把"GitHub Actions / CI/CD" 这个概念单独拆出来讲透。本笔记定位是"以 GitHub Actions + Quartz 部署为主案例的 CI/CD 入门",其他 CI 工具如 GitLab CI / Jenkins 只做横评,不展开。后续若需扩展:可加 § 11 安全专题(SAST/DAST/SCA/supply chain
- 除上述外未标注其他外部出处；本知识库内容主要来自与 AI 的多轮交流沉淀（见项目 CLAUDE.md 的定位说明）。
- 正文引用的外部域名：`github.com`
...
| 维度 | 客观情况 |
|---|---|
| 时效性 | 含约 14 处具体版本号/型号表述；含约 1 处价格相关表述；文件最后修改于 2026-06-07——版本与价格类信息变动快，引用前请核对当前情况 |
| 覆盖范围 | 全文约 14k 字，覆盖范围以正文目录为准，未展开的边界情况请另行查证 |
| 实战验证 | 正文含约 2 处实测/复现/踩坑类表述（自动统计，仅供参考；具体哪些结论经过动手验证需读正文判断） |
