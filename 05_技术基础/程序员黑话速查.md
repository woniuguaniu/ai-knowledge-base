...
- [A-Z 英文术语](#a-z-英文术语)
  ...
  - [CRUD](#crudcreate-read-update-delete增删改查)
  ...
  - [LoRA](#loralow-rank-adaptation低秩适配)
  - [MaaS](#maasmodel-as-a-service模型即服务)
  - [MCP](#mcpmodel-context-protocol)
  - [mcp-server](#mcp-server)
  - [MRR / ARR](#mrr--arrmonthly--annual-recurring-revenue月--年度经常性收入)
  ...
  - [SLA](#slaservice-level-agreement服务级别协议)
...
**一句话理解**：对数据库表的 **4 个基本操作** — 增加、查询、修改、删除。

**生活类比**：**通讯录管理**

```
Create  → 添加新联系人
Read    → 查看联系人列表 / 查看某个联系人详情
Update  → 修改联系人的电话号码
Delete  → 删除联系人
```
...
| CRUD | HTTP 方法 | SQL | 例子 |
|------|----------|-----|------|
| **C**reate | POST | INSERT | 新建一条 SLA 配置 |
| **R**ead | GET | SELECT | 查询所有用户 / 查询单个用户 |
| **U**pdate | PUT / PATCH | UPDATE | 修改用户信息 |
| **D**elete | DELETE | DELETE | 删除用户 |
...
GET    /api/users       // Read   - 列出所有用户
GET    /api/users/:id   // Read   - 查询单个用户
PUT    /api/users/:id   // Update - 更新用户信息
DELETE /api/users/:id   // Delete - 删除用户
```

**为什么重要**：
- **90% 的后端开发就是写 CRUD** — 增删改查是最基础的数据操作
- 面试常问："你做过什么项目?" → "写了个 XX 管理系统" → 本质就是 CRUD
- RESTful API 设计的核心就是把 CRUD 映射到 HTTP 方法

**常见变体**：
- **CRUDL**：多了个 **L**ist(列表查询)，把 Read 拆成"查单个"和"查列表"
- **软删除**：Delete 不是真删，而是标记 `deleted_at` 字段，数据还在
...
**应用场景**：[Agent 安全攻防 § 4.6](../06_Agent工程/Agent安全攻防.md#46-人在回路hitl) 12 项防御技术之一，最可靠的兜底防线。
...
**一句话理解**：**给大模型"打补丁"式的省钱微调**——原模型冻结不动，只在旁边训练一个很小的附加层。

**生活类比**：**给衣服打补丁，而不是重做一件**。原版大模型是一件昂贵西装，想让它多个口袋（学个新本事），不必重缝整件，只在外面缝一小块补丁；补丁单独存、随时换。

**省在哪**：

- 原模型几十上百亿参数全部**冻结**（不参与训练）
- 只训练两个"瘦长"小矩阵——它们的乘积去近似"本该做的那一大坨改动"，这就是名字里的 low-rank（低秩）
- 实际训练参数通常 < 1%，显存大降；产出的补丁只有 MB 级（原模型是 GB 级），同一底座能挂不同补丁随时切换

**和它常一起出现**：[QLoRA](#qloraquantized-lora量化版)——再省一截显存的升级版。

**应用场景**：原理详见 [模型训练技术速查 § 2.6](../00_核心概念/模型训练技术速查.md#26-省显存的微调lora-与-qlora)，讲它在 SFT 微调里怎么用。
...
### QLoRA（Quantized LoRA，量化版）

**一句话理解**：**[LoRA](#loralow-rank-adaptation低秩适配) 再省一截**——先把要打补丁的底座模型压缩到 4-bit，再在压扁的底座上训练 LoRA 补丁。

**生活类比**：先把那件西装**真空压缩打包**（量化，体积砍到约 1/4），再在上面缝补丁（LoRA）。两步都为省地方。

> **QLoRA = 把底座压扁（量化）+ 只训小补丁（LoRA），两个省显存的招叠一起。**

**效果**：原本要几张专业卡才能微调的大模型，**一张消费级显卡（如 24G 的 4090）就能微调**。

**应用场景**：原理与"量化那一半"见 [模型训练技术速查 § 2.6](../00_核心概念/模型训练技术速查.md#26-省显存的微调lora-与-qlora) 和 [本地部署模型量化选型](../03_应用实践/本地部署模型量化选型.md)。
...
**一句话理解**：数据库表的 **"结构定义"** — 有哪些字段、每个字段是什么类型、有什么约束。

**生活类比**：**Excel 表的表头 + 每列的数据类型规则**

```
Excel 表:
| 姓名(文本) | 年龄(数字) | 入职日期(日期) | 是否在职(是/否) |
|-----------|-----------|--------------|----------------|
...
CREATE TABLE users (
  ...
  INDEX idx_email (email)                        -- 邮箱索引,加速查询
);
```
...
| 组成部分 | 说明 | 例子 |
|---------|------|------|
| **字段名** | 列的名字 | `username` / `email` |
| **数据类型** | 存什么类型的数据 | `INT` / `VARCHAR` / `TIMESTAMP` |
| **约束** | 规则限制 | `NOT NULL`(不能为空) / `UNIQUE`(唯一) / `PRIMARY KEY`(主键) |
| **默认值** | 不填时的默认值 | `DEFAULT CURRENT_TIMESTAMP` |
| **索引** | 加速查询的字段 | `INDEX idx_email (email)` |

**为什么重要**：
- **Schema 是数据库的"合同"** — 定义了数据的形状,前后端都要遵守
- **改 Schema 很危险** — 线上表有数据时,改字段类型 / 加 NOT NULL 可能导致服务中断
- **Schema 设计影响性能** — 索引加对了查询快 100 倍,加错了写入慢 10 倍

**常见场景**：
- 后端开发:"先设计 Schema,再写 CRUD API"
- 数据库迁移:"写 migration 脚本修改 Schema"
- ORM 框架:"从代码自动生成 Schema"(如 Django / TypeORM)

**延伸**：
- **Schema-less / NoSQL**:MongoDB 等文档数据库"没有固定 Schema",每条记录可以有不同字段
- **Schema 版本管理**:用 Flyway / Liquibase 等工具管理 Schema 变更历史
...
工单系统的 SLA:
- P0(紧急故障) → 15 分钟内响应, 2 小时内解决
- P1(高优先级) → 1 小时内响应, 8 小时内解决
- P2(普通问题) → 4 小时内响应, 24 小时内解决
```
...
| 场景 | SLA 内容 |
|------|---------|
| **云服务商** | AWS 承诺 EC2 可用性 99.99%(每月停机不超过 4.38 分钟) |
| **客服系统** | VIP 客户 5 分钟响应,普通客户 1 小时响应 |
| **API 服务** | 99.9% 可用性 + P95 延迟 < 200ms |
| **运维团队** | 生产故障 30 分钟内定位,2 小时内修复 |

**为什么重要**：
- **量化服务质量** — 不是"尽快处理",而是"15 分钟内"
- **责任边界清晰** — 超时了谁负责、怎么赔偿
- **资源分配依据** — P0 工单配 3 个人,P2 工单配 1 个人

**SLA 的三个核心指标**：

| 指标 | 说明 | 例子 |
|------|------|------|
| **响应时间** | 多久内开始处理 | 15 分钟内有人接单 |
| **解决时间** | 多久内彻底解决 | 2 小时内修复上线 |
| **可用性** | 服务正常运行的时间占比 | 99.9% = 每月停机 < 43 分钟 |

**为什么要配置化**：
- **不同客户 SLA 不同** — VIP 客户 15 分钟,普通客户 1 小时
- **不同工单类型 SLA 不同** — 故障工单 vs 咨询工单
- **写死在代码里** → 改一次要重新部署
- **放配置表** → 运营人员自己在管理后台改

**常见误区**：
- ❌ "SLA 越短越好" → 成本指数级上升(5 分钟响应需要 24 小时值班)
- ❌ "SLA 是给客户看的" → 也是内部资源分配 / 绩效考核的依据
- ❌ "违反 SLA 就赔钱" → 有些 SLA 只是内部目标,不涉及赔偿

**延伸**：
- **SLO**(Service Level Objective,服务级别目标):内部目标,比 SLA 更严格
- **SLI**(Service Level Indicator,服务级别指标):实际测量值,如"本月可用性 99.95%"
...
- [MRR / ARR](#mrr--arrmonthly--annual-recurring-revenue月--年度经常性收入) — SaaS 核心收入指标
...
| 从需求到产品完成的工程流水线 | [软件工程产品研发 SOP](软件工程产品研发SOP.md) | 把 PRD / MVP / PoC / TDD / CI/CD / GA 串成"需求判断 → Spec → 架构 → 计划 → 开发 → 验收 → 发布 → 反馈"的完整流程 |
| SSG / 静态部署术语的实战流程 | [静态站点生成器与 Quartz 部署实战](静态站点生成器与Quartz部署实战.md) | 速查表给 SSG / baseUrl / sitemap / try_files / rsync 等术语,新笔记把它们串成一次完整部署流程 |
| CI/CD / workflow / runner / artifact 术语的实战机制 | [CI/CD 与 GitHub Actions](CI-CD与GitHub-Actions.md) | 速查表给定义,新笔记给"推一次代码自动构建测试部署通知"的完整机制 + workflow yaml 详解 + 主流 CI 工具横评 |
