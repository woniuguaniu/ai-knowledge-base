...
- [3. Tensor Core 代际演进:精度倍率全表](#3-tensor-core-代际演进精度倍率全表)
  ...
  - [3.6 Blackwell:FP4 与"AI TOPS"营销](#36-blackwellfp4-与ai-tops营销)
...
1. **同样跑 14B 量化模型**,3090 24G 流畅、4060Ti 16G 却卡顿——**为什么?**
...
**到了 LLM 上,瓶颈基本相反了——主要卡在带宽,不太卡算力**(更准确说:**单流 / 小 batch 的逐 token 解码**阶段卡带宽;而**处理输入的 prefill 阶段、以及大 batch 高并发服务**仍然吃算力)。
...
| 跑 7B 模型 | 慢 | 快 |
| 跑 14B 模型 | 勉强(慢)| 流畅 |
| 跑 70B 量化 | ❌ 跑不动 | ⚠️ 勉强(需极限低比特,见下) |

**结论**:**3090 的 LLM 体感 ≈ 4060Ti 16G 的 3~5 倍**。**别看显存差不多,带宽差距巨大**。

> ⚠️ **关于"3090 跑 70B"的实话**:单张 3090 只有 24GB,而 70B 模型**即使 4-bit 量化,光权重就要 35-45GB**——单卡根本放不下。真要在单卡硬跑只能上 **IQ2/Q2 这种极限低比特**(约 20GB,速度和质量都打折);正常做法是 **2×3090(分片合计 48GB)** 或 **把一部分层 offload 到内存**(很慢)。上表的"⚠️ 勉强"只是**相对 4060Ti** 而言,不代表单张 3090 能舒服跑 70B。
...
你的预算与目标:
  ...
  │     └─→ 2x 3090 NVLink(合计 48GB,模型分片跨卡,非单一显存池)
...
| 不带桥的 3090 | 配 4-slot NVLink 桥 | 多卡训练跨卡通信吃紧;但 3090 NVLink 112.5 GB/s ≈ PCIe 4.0 x16(64 GB/s)的 1.8 倍,并非 8 倍 |
...
| 维度 | 局限 |
|---|---|
| 时效性 | 显卡型号、显存价格、二手行情变动快；架构代际（Ada / Ampere / Hopper 等）相对稳定，**具体型号推荐与价格判断易过期** |
| 数据来源 | 算力数字、显存带宽等来自公开规格表；**未做实机跑分**，实际性能受散热、功耗墙、驱动版本影响 |
| 覆盖范围 | 以 NVIDIA 消费级/专业卡为主；AMD、Intel、国产加速卡（昇腾、寒武纪）与云厂商自研芯片未系统覆盖 |
| 表达风格 | 部分表述带社区玩梗色彩，**洞察可信但措辞不严谨**，正式场合引用前请核对原始规格 |
