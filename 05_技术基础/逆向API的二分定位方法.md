...
> 来源：2026-04 某中转 API 的逆向分析。**原始探索记录未入库**——当时的完整问答与请求日志没有归档到 `04_探索日志/`，本文是事后凭记忆整理的方法骨架，具体数字（请求数、耗时）为估算。
...
| 相关笔记 | 关系 |
|---|---|
| [Apifox](Apifox.md) | 互补关系：本文处理**没文档的黑盒接口**，Apifox 管理**已有文档的接口**。逆向摸清规则后，用 Apifox 把"最小可通过请求"固化成文档和回归测试用例，避免规则变更后重新从零测 |
| [LLM接口规范实战](../03_应用实践/LLM接口规范实战.md) | 对照阅读：那篇是**公开规范**下的接口长相，本文是**规范缺失**时如何反推出规范。知道标准接口该长什么样，能大幅缩小二分的搜索空间 |
| [LLM典型失败模式](../06_Agent工程/LLM典型失败模式.md) | 方法论同源：本文的「把恰好能跑当成协议本意」和「灰度让结论看起来矛盾」，本质是**样本不足就下结论**——和 LLM 的表演性完成是同一类认知陷阱 |
| [程序员黑话速查](程序员黑话速查.md) | 本文出现的灰度、中间层、状态码语义等术语，在这篇有通俗解释 |
| [Shell与终端基础知识总结](Shell与终端基础知识总结.md) | 二分测试时大量使用 `curl` 构造请求，这篇补齐命令行与管道基础 |
...
- 来源：2026-04 某中转 API 的逆向分析。**原始探索记录未入库**（`04_探索日志/` 无对应条目），正文数字为事后估算
