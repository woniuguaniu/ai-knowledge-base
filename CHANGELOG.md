...
- 06 模块 README 主题分组 13 → 6（原有 7 个分组各只含 1 篇，分类过细）。
- 修复 4 处 UTF-8 字符损坏；`index.md` 模块列表补上 07（此前写「七大模块」但实际已有 8 个）。
...
- **补齐「来源与局限性」章节**：50 篇笔记，覆盖率 29/79 → 79/79。来源只采信正文中真实存在的出处标记（12 篇提取到明确出处），38 篇正文确无出处者如实标注「未记录」而非编造；局限性只陈述可统计的客观事实（版本号/价格表述数量、字数、实测表述数量），不做脚本无法验证的定性判断。首次批量写入因「已有章节」检测正则过窄导致 73 篇重复追加，已整体回退重做。
