...
KV Cache 大小 = 4096 × 96 × 128 × 96 × 2 (K和V) × 2 (字节)
             ≈ 19 GB（≈18 GiB，单条序列 batch=1）
             ⚠️ 并发 batch=8 时再 ×8 ≈ 150 GB（这才是"显存爆炸"的实际来源）
...
> ⚠️ **但 DeepSeek-V3 的招牌创新恰恰是「无辅助损失（auxiliary-loss-free）」负载均衡**：
> 不再靠 aux_loss 的梯度去硬推平负载（那会损害主任务性能），而是给每个专家加一个**可动态调整的偏置项**——
> 某个专家被选少了就调高它的偏置、被选多了就调低，从而在**不引入额外损失梯度**的前提下让负载自动均衡（V3 仅保留极小的序列级均衡损失兜底）。
...
它要解决传统 RLHF 中 **PPO 算法**的核心痛点：**要额外训一个独立的 Value Model（价值模型 / Critic）** 来估计「优势函数」——这个 Critic 和主模型一样大，**显存占用直接翻倍**。

> ⚠️ GRPO 真正省掉的是 **Critic（价值模型）**，**不是奖励信号本身**。DeepSeek-R1 的推理 RL 进一步用**规则奖励**（答案对错 / 格式合规）替代了神经网络奖励模型，但那是「结果可自动判分」任务的特例，并非「GRPO 一定不需要奖励模型」。
...
> **一句话**：DeepSeek-R1 系列用 **RL 为主**的方式**让模型自己学会"思考链"**，是开源社区第一个对标 OpenAI o1 的推理模型，并把"思考过程"完整公开。其中 **R1-Zero 是纯 RL（零 SFT 冷启动）**，而**生产版 R1 是多阶段训练（冷启动 SFT → 推理 RL → 拒绝采样 SFT → 全场景 RL），并非纯 RL**。
...
- 参考资料：DeepSeek-V2 技术报告（MLA）
- 参考资料：DeepSeek-V3 技术报告（MoE 架构、训练框架）
- 参考资料：DeepSeek-R1 技术报告（GRPO、多阶段训练、Aha Moment、蒸馏）
- 参考资料：DeepSeek-Math 论文（GRPO 算法首次提出）
...
| 维度 | 客观情况 |
|---|---|
| 时效性 | 含约 41 处具体版本号/型号表述；文件最后修改于 2026-06-03——版本与价格类信息变动快，引用前请核对当前情况 |
