...
| 维度 | 局限 |
|---|---|
| 定位 | 本篇是**入门骨架**（约 100 行），只讲 RAG 是什么、为什么、基本流程。工程细节都在配套篇：三件套选型见[Embedding-Reranker-向量数据库](../Embedding-Reranker-向量数据库.md)，知识层范式见[LLM-Wiki知识库架构](LLM-Wiki知识库架构.md) |
| 覆盖缺口 | 切块策略、Hybrid Search、多路召回、RAG 评测指标等均未在本篇展开 |
| 时效性 | 概念层稳定；但"当前主流做法"随生态变化，2026 年已出现 PageIndex / Vectorless 等替代路线（见 LLM-Wiki 篇） |
| 实战验证 | 内容为学习整理，**未附可跑通的完整工程**；代码级实现请看配套篇的 Pipeline 示例 |
