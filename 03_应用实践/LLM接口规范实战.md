...
>
> **适用时间**：截至 2026-05-29 的知识库整理版本。接口范式和常见坑具备参考价值；具体 endpoint、字段名、SDK 行为和 beta API 状态必须以各家官方文档为准。
...
| 相关笔记 | 关系 |
|---|---|
| [Apifox](../05_技术基础/Apifox.md) | 工具配套：本文讲各家接口的字段规范长什么样，Apifox 用来把这些接口**管起来**（文档、调试、Mock、回归测试） |
| [逆向API的二分定位方法](../05_技术基础/逆向API的二分定位方法.md) | 反向情况：本文是**有官方规范**时怎么读，那篇是**规范缺失或与实现不符**时怎么反推。本文提到的"官方文档滞后于实现"正是那套方法的用武之地 |

### 9.5 局限性

| 维度 | 局限 |
|---|---|
| 时效性 | **高风险**——四家接口都在演进：OpenAI Response API 字段仍在变、Gemini 仍挂 `/v1beta`、Anthropic 的 `cache_control` 持续增强。**字段级细节必须以官方 changelog 为准** |
| 覆盖边界 | 只讲 **HTTP 层**，不讲各家 SDK 的封装差异；Function Calling 与 MCP 的完整用法在[Function Calling与MCP工程指南](../06_Agent工程/Function%20Calling与MCP工程指南.md) |
| 厂商范围 | 聚焦御三家 + OpenAI 兼容生态，国内厂商（豆包、混元等）的私有扩展字段未系统覆盖 |
| 实战验证 | 多模态、思考模型字段等部分为资料整理，整理者**未逐一发请求验证**；`reasoning_content` 等非标准字段的支持情况因厂商而异 |

---

*最后更新：2026-08-29（新增 § 10 关联章节，回链 Apifox 与逆向 API 二分定位法；正文内容未改动，此前正文日期为 2026-06-02）*
