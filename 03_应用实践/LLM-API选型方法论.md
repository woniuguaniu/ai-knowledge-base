...
>
> **适用时间**：截至 2026-05-29 的知识库整理版本。五维选型框架可以长期复用；API 供应商、模型价格、发票/合规政策和平台稳定性属于高时效信息，落地前必须重新核对。
...
| 维度 | 局限 |
|---|---|
| 时效性 | **最需警惕**——原文写于 2026-01，18 家提供商的价格、限流、可用模型变动极快；表中数据引用前必须到各家官网核对 |
| 地域范围 | 场景案例与提供商清单以**国内可用**为前提，涉及数据出境、海外合规的场景未覆盖 |
| 视角 | 5 维度框架来自单一作者的实践，偏**中小团队/个人**视角；大型企业的采购流程、SLA 谈判、私有化条款不在其中 |
| 实战验证 | 整理者未逐家实测；3 个场景案例转述自原作者经历，未独立复现 |
| 已知缺口 | 见 § 8.4「作者没覆盖到的」——行业垂直场景、数据出境合规等 7 类方向仍是空白 |
