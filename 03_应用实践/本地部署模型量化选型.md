...
- **微调时也用到这里的量化**：本篇的 4-bit / NF4 量化，正是 [模型训练技术速查 § 2.6](../00_核心概念/模型训练技术速查.md#26-省显存的微调lora-与-qlora) 里 QLoRA"把底座压扁"那一半的细节
...
| 维度 | 局限 |
|---|---|
| 时效性 | 量化方法（GGUF / GPTQ / AWQ / EXL2 等）与显卡架构支持矩阵会随推理框架版本变化；本笔记整理于 **2026-05-24**，FP8 在 Ampere 上的支持情况等结论需按当前框架版本复核 |
| 实战验证 | 各量化方案的**显存占用与质量损失数据多为社区经验值**，整理者未在同一套硬件上做横向实测 |
| 覆盖范围 | 以单机/小规模部署为主；多机分布式推理、生产级 serving 的量化取舍见[LLM推理引擎选型](LLM推理引擎选型.md)与[HomeLab到中小企业LLM部署架构](HomeLab到中小企业LLM部署架构.md) |
| 硬件范围 | **NVIDIA 最详细**（按 Ada/Ampere/Turing 等架构逐代对号入座）；Apple Silicon 给到 MLX 首选指引但未展开调优；AMD ROCm 只有简要提及；国产加速卡（昇腾、寒武纪等）未涉及 |
