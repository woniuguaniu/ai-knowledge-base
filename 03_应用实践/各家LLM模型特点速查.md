...
>
> **适用时间**：截至 2026-05-29 的知识库整理版本。模型家族定位相对稳定，但具体型号、价格、上下文长度和 API 可用性变化很快，真实选型前必须再查官网 / 控制台 / 当前供应商文档。
...
| 维度 | 局限 |
|---|---|
| 时效性 | **本笔记时效性最差的一类**——模型型号、能力排名、价格半年内必然过期。所有具体型号名与"当前主力"判断都应视为**某个时间点的切片** |
| 主观性 | 大量内容是**社区个人体感**（"改 bug 用 codex / 新工程用 5.2"这类），不是基准测试。同一模型做不同任务体验差异巨大 |
| 覆盖范围 | 以中文社区关注的厂商为主，欧洲/日韩模型、垂直领域模型（医疗、法律）基本未覆盖 |
| 验证方式 | 整理者**未做横向基准测试**，内容为社区讨论与个人使用经验的汇总 |

> **正确用法**：当作"有哪些选手、各自大致什么性格"的地图，**不要当作选型结论**。真要选型，用自己的真实业务数据实测。
