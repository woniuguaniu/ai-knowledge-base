...
> ⏱️ **时效提示**：`/goal` 是 2026 年春季新增的功能，两家仍在快速迭代。本文对应 **Claude Code v2.1.139（2026-05-12）** 时的行为，其中**评估器用哪个模型、字符数上限、跨会话恢复方式**都可能已经变化。当作机制理解读，具体参数请以 `claude --help` 与官方 changelog 为准。
...
| 项 | 数值 |
|---|---|
| 最低版本 | Claude Code v2.1.139 起支持（本节数据对应 2026-05 版本，新版本请自行核对） |
| 完成条件最大长度 | 4000 字符 |
...
| 主题 | 文档 | 关系 |
|---|---|---|
| Claude Code 实战速查 | [Claude Code 实战速查.md](Claude%20Code%20实战速查.md) | 那篇的**命令速查表已收录** `/goal` 一行（命令怎么敲）；本篇讲的是它**背后的机制**（独立评估器循环）。查用法去那篇，理解设计看本篇 |
...
| 维度 | 局限 |
|---|---|
| 时效性 | **最需警惕**——`/goal` 是 2026 年春季新功能，两家仍在迭代。评估器用哪个模型、字符数上限、跨会话恢复方式都可能已变。§ 4–§ 6 描述的是 **v2.1.139（2026-05-12）当时的行为** |
| 信息来源 | 内部机制（Haiku 评估器、循环逻辑）来自**社区评测博客与官方公告的交叉印证**，不是官方架构文档；实现细节可能有出入 |
| 实战验证 | 整理者**未做长时间成本实测**；"一日可烧几十美元"的警示来自社区反馈，非本人账单数据 |
| 对比范围 | 与 OpenAI Codex `/goal` 的对比基于公开信息，Codex 侧的评估器实现**未公开**，该行对比只能停在"未知" |
