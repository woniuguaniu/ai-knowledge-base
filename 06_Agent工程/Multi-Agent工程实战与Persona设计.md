...
### 2.1.1 适用边界：长期角色流水线 ≠ 临时编码子代理

> 本案例证明的是**特定长周期流水线**可以受益于角色分工，**不代表编码任务默认应该拆成 subagent**。

这两件事经常被混为一谈，但适用条件完全不同：

| | 本文的 Multi-Agent | Coding Agent 的临时子代理 |
|---|---|---|
| 存在周期 | 长期存在，定时运行 | 单次任务，做完退出 |
| 划分依据 | **角色**（分析师 / 风控 / 交易员…） | **任务**（3 个互相独立的子任务） |
| 核心收益 | 专业化分工 + 流水线并行 | 保持主上下文干净 |
| 何时不该用 | 业务本身没有角色分化 | **单 Agent 已经够用时** |

关于第二列的反方证据：有研究指出，**对于工具密集、推理链长的任务，单 Agent 表现已经足够好，某些任务引入多 Agent 效果反而更差**（arXiv《Towards a Science of Scaling Agent Systems》）；开源极简 Coding Agent **pi** 的作者更直接把 subagent 称为 *"a black box within a black box"*（黑盒里的黑盒）。详细论证与替代方案见 [极简可控的Coding-Agent设计-pi](极简可控的Coding-Agent设计-pi.md)。

**判断口径**：单 Agent 足够时，不要为了组织形式而增加编排层——编排本身有成本（写合约、开 worker、review、结果合并）。
...
- **[Skill 工程化设计与失效防御.md](Skill工程化设计与失效防御.md) § 5.3** — **不同层次，注意区分**：那篇讲的是**单次工程任务里的临时子代理**（五字段合约派发、两阶段 review、做完退出），本文讲的是**长期存在、定时运行、角色化的业务流水线**。两者都叫「多 Agent」，但一个是任务分解手段、一个是组织形态，不要混用结论。
...
| 维度 | 局限 |
|---|---|
| 单一案例 | 全文建立在**一个 7 人 AI 团队炒股案例**上。它证明了这套做法在该场景可行，**不构成对 Multi-Agent 普遍有效的证据**——反向边界见 § 2.1.1 |
| 策略效果有限 | 原作者**已披露收益数据且明确说不理想**——正文 § 2.1 引其原话「正在优化的那条策略年化只有 14%，离 50% 目标还很远」。所以本案例证明的是**工程架构可行**，不是**策略赚钱**，两者不要混为一谈 |
| 领域特殊性 | 量化交易有天然优势：目标可量化、反馈及时、数据结构化。**多数业务场景不具备这三条**，照搬前先问自己有没有 |
| 实战验证 | 整理者**未复现该系统**，内容为原文吸收 + 交叉分析；四大原则与三大教训的可迁移性未经本人验证 |
| 时效性 | Persona 写法与模型分级策略绑定 2026 年中的模型能力与价格，会随模型迭代变化 |
