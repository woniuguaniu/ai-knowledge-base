...
- 阶梯论真正描述的是**瓶颈的迁移**（模型弱时措辞是瓶颈 → 中等时上下文拼装是瓶颈 → 很强时外围工程结构是瓶颈，见该篇对四个阶段的划分），不是技术的代际淘汰
...
## 7.5 边界案例：表现层与 Agent runtime 的解耦原型

上面的组件清单默认一个前提：**harness 是一整套绑在一起的东西**。但 2026 年出现了一个反例，值得记下来作为组件边界的参照。

> **案例（2026-08）**：开源项目 [`BytePioneer-AI/codex-host`](https://github.com/BytePioneer-AI/codex-host) 把 Codex Desktop 当作 Host / UI 层，后端可替换为 **pi、Claude Code、Grok Build、DSH、OMP**，已实现工具调用、问题弹窗、Diff、Fork 等交互。作者动机很朴素：喜欢 pi 但它没有官方 GUI、只能用终端，而 Codex GUI 的交互体验用惯了回不去了。

**它说明的是**：在**工具调用、diff 呈现、权限询问、会话分叉**这几件具体交互上，各家已经存在一个**可映射的最小公分母**——足以让一个适配层把不同后端塞进同一个前端。

**它不说明的是**：

- 不代表协议已经标准化（没有共同规范，只是行为恰好可映射）
- 不代表 harness 整体可替换——这里解耦的是**狭义的 runtime 与表现层**，而本文全篇用的是广义定义（harness = 模型以外的一切），后者包含的上下文治理、权限策略、错误恢复并不随 GUI 一起搬走
- 样本只有单个社区项目、发布仅数日，长期维护性未经验证

顺带一个观察：这也是"**极简内核**"策略的意外红利。pi 砍掉 GUI 保持核心极小，反而让别人能给它接任意前端；自带深度耦合 TUI 的工具，做这种适配会难得多（见 [极简可控的Coding-Agent设计-pi](极简可控的Coding-Agent设计-pi.md)）。
...
- **词源考据**：《什么是 Harness Engineering：聊聊最近爆火的技术词》（2026 年年中，作者自述综合 linux.do 20+ 帖子与多篇一手文献）——§ 1.1 时间线与 § 1.2 三种口径均据此
- **架构与实践**：Anthropic / OpenAI 公开工程报告、Claude Code 源码泄露事件的社区分析、swyx 的 IMPACT 框架
- **社区实践**：§ 5.5 的六层视角对照、§ 7.6 的八大器官对照为多篇社区讨论的归纳
- **本知识库补充**：与 AI 的多轮讨论整理
...
| 维度 | 局限 |
|---|---|
| 术语未定型 | **harness 这个词本身还在流动**——§ 1.2 列的三种解释口径至今没有权威定义。引用本文的定义时请注明是哪一种口径 |
| 来源性质 | 大量内容基于**社区考据与源码泄露分析**，不是厂商官方架构文档；具体实现细节可能与真实代码有出入 |
| 时效性 | 架构层认识相对稳定；但涉及具体产品行为（Claude Code 的 hook 事件、压缩阈值等）随版本变化，§ 7.5 的生态案例更是数日内的快照 |
| 覆盖边界 | § 4.3 已声明五层架构图的局限（那是**模型的局限**）；本节说的是**本笔记的局限**——两者不同 |
| 实战验证 | 整理者未实现过完整 harness，理解来自资料研读与使用体验 |

---

*最后更新: 2026-08-30（新增 § 7.5 表现层与 runtime 解耦原型案例、§ 「来源与局限性」章节；正文其余部分对应 2026-07-06 那次更新：§ 1.1 词源考据时间线 + swyx IMPACT 前传、§ 1.2 三种解释口径、§ 2.1 三层楼板视角、§ 2.2 通用/项目/任务三层划分、§ 7.6 八大器官对照，并修正 § 3 "泄露=词源"叙事为"泄露=催化剂"）*
*归属模块: 06_Agent工程*
