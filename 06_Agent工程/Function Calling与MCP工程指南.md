...
##### 1. 工具描述边界不清 → LLM 在相似工具间摇摆

**反例**（两个工具描述都太模糊）:
```python
# ❌ search_docs: "搜索项目文档"
# ❌ search_code: "搜索代码仓库"
# 用户问"登录接口的 token 校验逻辑在哪"，两个工具看起来都能用
```

**正例**（描述要回答三问：解决什么 / 何时用 / **何时不用**）:
```python
{
  "name": "search_code",
  "description": "在代码仓库中查找函数、类、变量、接口实现位置。适合定位实现逻辑、调用链、字段定义。**不用于查询线上日志和业务数据**。"
}
{
  "name": "query_logs",
  "description": "查询线上服务日志。适合排查报错、超时、异常请求。**不用于搜索代码实现或读取配置文档**。"
}
```

##### 2. 工具功能重叠 → 出现"工具碰撞"

**场景**：企业办公 Agent 有三个工具——查飞书文档、查知识库、查历史工单。用户问"报销额度调整说明在哪"，三个都可能有，LLM 会来回切换、结果混乱。

**应对**：
- **定权威源排序**：制度类优先查知识库 → 协作过程类优先查飞书 → 历史处理类查工单
- **后端合并工具**：三个数据源在后端合成一个 `retrieve_knowledge`，内部按来源/权限/时间做检索分发，对 LLM 只暴露一个清爽接口

原则：**不要把后端系统的复杂性原样甩给模型**——模型适合做语义判断，不适合在工程细节里猜你的系统边界。

##### 3. 参数 schema 太宽 → LLM 会乱填、甚至越权
...
```python
{
  ...
  "description": "查询业务数据库",
  "parameters": {
    "sql": {"type": "string"}  # ❌ LLM 可以写任意 SQL
  }
}
# 用户问"查一下所有用户的退款记录"，LLM 可能生成 SELECT * FROM users 越权查询
```
...
```python
{
  ...
  "description": "查询**当前登录用户**的订单状态",
  "parameters": {
    "date_range": {
      "type": "string",
      "enum": ["last_7_days", "last_30_days", "custom"]  # ✅ 枚举限制
    },
    "order_id": {"type": "string", "description": "可选，指定订单号"}
  }
}
# 真正的 SQL 由服务端根据当前用户身份生成，权限在服务端校验
```

**核心原则**：让模型做"想干什么"（意图 + 参数提取），让确定性代码做"能不能干、怎么安全干"（权限、执行、边界控制）。

##### 4. 调用后没验证 → 陷入工具循环或抓错证据

**场景**：Agent 为了排查接口超时，调日志工具查到 200 行日志，里面有些 warning 但没有真正的 error。如果没有观察判断，LLM 可能抓住无关 warning 乱归因。
...
第 2 步：换关键词再查日志
第 3 步：继续查日志
第 4 步：还是查日志（一直在行动，但无信息增量）
```

**应对**：
- **每次调用后做结构化判断**：这次得到的信息是什么？和原问题有什么关系？是否足够进入回答阶段？
- **设终止条件**：
  - 连续两次工具结果高度重复 → 提示模型停止同类调用
  - 连续三次没有新证据 → 强制进入总结或请求用户补充信息
  - `max_steps` 只是兜底，真正有用的是**信息增量判断**
