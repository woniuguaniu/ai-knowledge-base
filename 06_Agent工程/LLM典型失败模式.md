...
| 关联点 | 文档 | 关系 |
|---|---|---|
| 压力下自我合理化 | [Skill工程化设计与失效防御](Skill工程化设计与失效防御.md) | **同一问题的独立发现**：那篇的 Red Flags 与「借口表只能从真实失败里抄」，对应本文的表演性完成；两边都指向 Agent 会为跳步编造理由 |
...
| **写代码场景的表演性完成** | [Vibe Coding 与技术债治理](Vibe%20Coding与技术债治理.md) | 本文「表演性完成」在 AI 写代码时的具体表现就是"能跑≠对"；该文给出 vibe coding 技术债的 6 条治理对策 |
| **可观测性优先的工具取向** | [极简可控的 Coding Agent 设计：pi](极简可控的Coding-Agent设计-pi.md) | pi 的「拒绝黑盒、一切可见」哲学呼应本文——只有看得见 Agent 真正做了什么，才能识破"表演性完成" |
...
| 维度 | 客观情况 |
|---|---|
| 时效性 | 文件最后修改于 2026-08-29 |
| 覆盖范围 | 全文约 6k 字，覆盖范围以正文目录为准，未展开的边界情况请另行查证 |
| 实战验证 | 正文含约 4 处实测/复现/踩坑类表述（自动统计，仅供参考；具体哪些结论经过动手验证需读正文判断） |
