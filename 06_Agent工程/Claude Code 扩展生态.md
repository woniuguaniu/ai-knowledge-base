...
>
> **适用时间**：截至 2026-05-29 的知识库整理版本。扩展机制的概念相对稳定；具体命令、目录约定、Plugin 市场和 MCP 管理方式需要以当前 Claude Code 版本为准。
...
| **极简反例：pi** | [极简可控的Coding-Agent设计-pi.md](极简可控的Coding-Agent设计-pi.md) | **设计取向相反的对照**：CC 把 Skill/MCP/SubAgent/Hook 五大机制内置；pi 把它们大多砍掉、下放成"按需扩展"，主打可视化/极简/可控 |
...
- 正文**未标注外部出处**。本知识库的内容主要来自与 AI 的多轮交流沉淀（见项目 CLAUDE.md 的定位说明），但本文对应的具体对话时间与原始材料**未记录**。
- 正文引用的外部域名：`lobehub.com`、`jimeng.jianying.com`、`smithery.ai`、`open.feishu.cn`、`www.firecrawl.dev`
...
| 维度 | 客观情况 |
|---|---|
| 时效性 | 含约 34 处具体版本号/型号表述；文件最后修改于 2026-08-29——版本与价格类信息变动快，引用前请核对当前情况 |
| 覆盖范围 | 全文约 12k 字，覆盖范围以正文目录为准，未展开的边界情况请另行查证 |
